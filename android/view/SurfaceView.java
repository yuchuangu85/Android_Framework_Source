...
/**
 * Provides a dedicated(专用的) drawing surface（表面，外观） embedded（嵌入） inside of a view hierarchy.
 * You can control the format（版式） of this surface and, if you like, its size; the
 * SurfaceView takes care of placing the surface at the correct location on the
 * screen
 *
 * <p>The surface is Z ordered so that it is behind the window holding its
 * SurfaceView; the SurfaceView punches（冲压） a hole in its window to allow its
 * surface to be displayed. The view hierarchy（层次结构） will take care of correctly
 * compositing with the Surface any siblings（同族） of the SurfaceView that would
 * normally appear on top of it. This can be used to place overlays（覆盖物） such as
 * buttons on top of the Surface, though note however that it can have an
 * impact（影响） on performance since a full alpha-blended（混色） composite will be performed
 * each time the Surface changes.
 *
 * <p> The transparent（透明） region that makes the surface visible is based on the
 * layout positions in the view hierarchy. If the post-layout transform（变形）
 * properties are used to draw a sibling（同族） view on top of the SurfaceView, the
 * view may not be properly composited with the surface.
 *
 * <p>Access to the underlying surface is provided via the SurfaceHolder interface,
 * which can be retrieved（检索，恢复） by calling {@link #getHolder}.
 *
 ...
 * this way, you need to be aware of some threading semantics（语义，语法）:
 *
 * <ul>
 * <li> All SurfaceView and
 * {@link SurfaceHolder.Callback SurfaceHolder.Callback} methods will be called
 * from the thread running the SurfaceView's window (typically（通常） the main thread
 * of the application). They thus（因此） need to correctly（正确的） synchronize（同步） with any
