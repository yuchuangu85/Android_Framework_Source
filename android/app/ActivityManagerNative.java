...
class ActivityManagerProxy implements IActivityManager {
    ...
    /**
     * 这里的registerReceiver最终通过Binder传递到ActivityManagerService中的registerReceiver方法
     *
     * @param caller      Binder服务端对象
     * @param packageName 包名
     * @param receiver    Binder服务端对象
     * @param filter      Selects the Intent broadcasts to be received.
     * @param perm        String naming a permissions that a
     *                    broadcaster must hold in order to send an Intent to you.  If null,
     *                    no permission is required.
     * @param userId      The userId stored in this UserHandle.
     *
     * @return
     *
     * @throws RemoteException
    ...
    public void activityPaused(IBinder token) throws RemoteException {
        ...
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeStrongBinder(token);
        // 通过Binder对象mRemote向AMS发送一个类型为ACTIVITY_PAUSED_TRANSACTION的进程间通信请求
    ...
    public void publishService(IBinder token,
        ...
        // 通过一个Binder代理对象mRemote向AMS发送一个类型为PUBLISH_SERVICE_TRANSACTION的进程间通信请求
    ...
    public ParceledListSlice<UriPermission> getPersistedUriPermissions(
        ...
        reply.readException();
        @SuppressWarnings("unchecked") final ParceledListSlice<UriPermission> perms = ParceledListSlice.CREATOR.createFromParcel(
                reply);
        data.recycle();
        reply.recycle();
        return perms;
