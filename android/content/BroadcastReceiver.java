...
public abstract class BroadcastReceiver {
    ...
    public static class PendingResult {
         ...
         *
         * 对于非order的动态广播而言，if和else if里面的条件都不满足，因此，finish函数就啥也不做
         */
        public final void finish() {
            if (mType == TYPE_COMPONENT) {
                ...
                // 对于接收者进程而言，order和非order广播都调scheduleRegisteredReceiver函数处理，
                // 只是上面处理的时候BroadcastReceiver.PendingResult中mOrderedHint为true，
                // mType是TYPE_REGISTERED，这时候在finish函数的时候，会调用sendFinished函数
                if (ActivityThread.DEBUG_BROADCAST) Slog.i(ActivityThread.TAG,
        ...
        public void sendFinished(IActivityManager am) {
            synchronized (this) {
                ...
                try {
                    ...
                    // 对于order广播而言，走的是if(mOrderedHint)这个分支，里面有的参数mAbortBroadcast代表
                    // 当前的高优先级的广播接收者是否需要disable低优先级的广播接收者接收当前的广播。
                    if (mOrderedHint) {
