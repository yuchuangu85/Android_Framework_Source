...
public class WearTouchMediatorTest {
    ...
    private @Mock EnablableInputDevice mockInputDevice;
    private @Mock WearTouchMediator.TouchInputProvider mockTouchInputProvider;

    private @Mock AmbientConfig mockAmbientConfig;
    private @Mock PowerTracker mockPowerTracker;
    private @Mock TimeOnlyMode mockTimeOnlyMode;

    private @Mock BooleanFlag mockUserAbsentTouchOffFlag;
    ...
    public void testOnBootCompleted() {
        ...
        assertThat(ShadowApplication.getInstance().hasReceiverForIntent(
    ...
    public void testHome() {
        ...
        mMediator.receiver.onReceive(mContext, new Intent(Home.ACTION_DISABLE_TOUCH));
        verify(mockInputDevice).disable();

        verifyNoOtherEnable(WearTouchMediator.Reason.OFF_HOME_REQUEST);

        mMediator.receiver.onReceive(mContext, new Intent(Home.ACTION_ENABLE_TOUCH));
        verify(mockInputDevice).enable();
    }
    ...
    public void testWetMode() {
        ...
        mMediator.receiver.onReceive(mContext, new Intent(WetMode.ACTION_WET_MODE_STARTED));
        verify(mockInputDevice).disable();

        verifyNoOtherEnable(WearTouchMediator.Reason.OFF_TOUCH_LOCK);

        mMediator.receiver.onReceive(mContext, new Intent(WetMode.ACTION_WET_MODE_ENDED));
        verify(mockInputDevice).enable();
    }
