...
public final class SystemServer {
    ...
    private void performPendingShutdown() {
        ...
        if (shutdownAction != null && shutdownAction.length() > 0) {
            ...
            // bugreport when that's still feasible（可行的）. (Bug: 26444951)
     ...
     * the system off the ground.  These services have complex（复杂） mutual（相互） dependencies
     * which is why we initialize them all in one place here.  Unless your service
     * is also entwined（纠缠在一起） in these dependencies, it should be initialized in one of
     * the other functions.
     */
    private void startBootstrapServices() {
        // Wait for installed to finish starting up so that it has a chance to
        // create critical（至关重要的） directories such as /data/user with the appropriate（适当的）
    ...
    private void startOtherServices() {
        ...
        if (mFactoryTestMode != FactoryTest.FACTORY_TEST_LOW_LEVEL) {
            ...
            if (!disableNetwork) {
                ...
                Trace.traceEnd(Trace.TRACE_TAG _SYSTEM_SERVER);
