...
import static com.android.server.am.TaskRecord.*;
...
public class ActivityStackSupervisor extends ConfigurationContainer implements DisplayListener,
    ...
    RecentTasks mRecentTasks;// 最近应用列表
    ...
    TaskRecord anyTaskForIdLocked(int id, @AnyTaskForIdMatchTaskMode int matchMode,
        ...
        if (matchMode == MATCH_TASK_IN_STACKS_OR_RECENT_TASKS) {// 这里我们需要保存所以跳过该步
            return task;
        }

        // Implicitly, this case is MATCH_TASK_IN_STACKS_OR_RECENT_TASKS_AND_RESTORE
        if (!restoreRecentTaskLocked(task, aOptions, onTop)) {// 保存该任务
    ...
    boolean restoreRecentTaskLocked(TaskRecord task, ActivityOptions aOptions, boolean onTop) {
        ...
        if (currentStack != null) {
            ...
            if (currentStack == stack) {// 这里判断任务栈是否在对的栈管理中。
